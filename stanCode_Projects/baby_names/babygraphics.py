        ...
        for i in range(len(YEARS)):
            if name in name_data:
                if str(YEARS[i]) in name_data[name]:
                    ...
                    rank_height = rank/1000 * (CANVAS_HEIGHT-2*GRAPH_MARGIN_SIZE) + GRAPH_MARGIN_SIZE
                    canvas.create_text(get_x_coordinate(CANVAS_WIDTH, i) + TEXT_DX,
                                       rank_height, text=(str(name) + str(name_data[name][str(YEARS[i])]))
                                       , anchor=tkinter.SW, fill=COLORS[count % len(COLORS)])
                    axis_y.append(rank_height)
                else:
                    canvas.create_text(get_x_coordinate(CANVAS_WIDTH, i) + TEXT_DX,
                                       CANVAS_HEIGHT - GRAPH_MARGIN_SIZE,
                                       text=(str(name) + " *"), anchor=tkinter.SW, fill=COLORS[count % len(COLORS)])
                    axis_y.append(CANVAS_HEIGHT - GRAPH_MARGIN_SIZE)
            if i > 0:
                canvas.create_line(get_x_coordinate(CANVAS_WIDTH, i), axis_y[i],
                                   get_x_coordinate(CANVAS_WIDTH, i-1), axis_y[i-1], fill=COLORS[count % len(COLORS)])
        count += 1
