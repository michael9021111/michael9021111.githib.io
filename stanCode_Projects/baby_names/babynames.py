...
def add_file(name_data, filename):
    ...
    with open(filename, 'r') as f:
        for line in f:
            born_baby = line.split(",")
            # ------------------------寫法一------------------------
            if len(born_baby) == 1:
                year = born_baby[0].strip()
            else:
                rank = born_baby[0].strip()
                name1 = born_baby[1].strip()
                name2 = born_baby[2].strip()
...
def search_names(name_data, target):
    ...
    for name in target_name:
        for baby in name_data:
            baby_lower = baby.lower()
            if name in baby_lower:
                names.append(baby)
    return names
